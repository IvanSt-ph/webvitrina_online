...
{{-- 🔍 Live-поиск товаров с подсветкой совпадений --}}
<div 
  x-data="{
    query: '',
    results: [],
    ...
    highlight(text) {
      ...
      const safeQuery = this.query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // экранирование
  ...
  class="relative w-full sm:w-96 order-last sm:order-none"
>
  {{-- Поле ввода --}}
  ...
  <div 
    ...
    class="absolute z-40 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl overflow-hidden max-h-96 overflow-y-auto backdrop-blur-sm"
  >
        ...
        @forelse($products as $product)
          <tr class="hover:bg-indigo-50/30 transition duration-200">
            ...
            <td x-show="columns.sku" class="px-4 py-3 font-mono text-xs text-gray-700 text-center">
              @if($product->sku)
                <div x-data="{ copied: false }" class="inline-flex items-center gap-1 group">
                  <span class="px-2 py-1 bg-gray-100 rounded border border-gray-200 cursor-pointer hover:bg-indigo-50 transition"
                        title="Скопировать SKU"
                        @click="navigator.clipboard.writeText('{{ $product->sku }}'); copied = true; setTimeout(() => copied = false, 1000)">
                    {{ $product->sku }}
                  </span>
                  <template x-if="copied">
                    <span class="text-green-600 text-[11px] font-semibold">✓</span>
                  </template>
                </div>
              @else
