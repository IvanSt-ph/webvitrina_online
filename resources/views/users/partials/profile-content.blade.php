    @php
        $shop = $user->shop;
        $isAdminPublicPreview = $isAdminPublicPreview ?? false;
    ...
    <div class="{{ $isAdminPublicPreview ? 'text-slate-900' : 'min-h-screen bg-slate-50 px-3 py-5 pb-24 text-slate-900 sm:px-5 lg:px-8 lg:py-8' }}">
                                    ...
                                    @auth
                                        @if($isAdminViewer)
                                            <a href="{{ route('admin.users.show', $user) }}"
                                               class="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 text-sm font-bold text-indigo-700 transition hover:bg-indigo-50">
